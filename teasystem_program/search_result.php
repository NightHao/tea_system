<html>
    ...
    <center>
        <body>
				...
				if(strcmp($brandname, "全部")!= 0){
					...
					//brandname不為空,shopname為空
						...
						//brandname不為空,shopname、name為空(法4)
							...
							$isjoin = 0;
							$query = ("select brandname,shopname,count(shopname) as c_shop  from brand where brandname = ? group by brandname");
							$stmt = $db->prepare($query);
							...
							for($i=0; $i < count($result); $i++){
								$brandname = $result[$i]['brandname'];
								$shopname = $result[$i]['shopname'];
								$c_shop = $result[$i]['c_shop'];
								brand_Row_c($brandname, $shopname, $c_shop);
								}
						}
					}
				}
			...
			function brand_Row_c($brandname, $shopname, $c_shop){
